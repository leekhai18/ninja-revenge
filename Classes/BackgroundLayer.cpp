...
bool BackgroundLayer::init()
{
	...
	Sprite* house0 = Sprite::create(HOUSE_2_PATH);
	...
	for (int i = 1; i < N_HOUSE; i++){
		Sprite* house = Sprite::create(HOUSE_1_PATH);
		int tag =  i - 1;
		house->setPosition(this->getChildByTag(tag)->getPosition().x + HOUSE_1_WIDTH*2, 
...
void BackgroundLayer::update(float delta){
	for (Sprite* &house : houses){
		if (house->getPosition().x < -house->getContentSize().width){			
			...
			int rand = random(1, 2);			
			if (rand == 1){
				house = Sprite::create(HOUSE_1_PATH);

			}
			else{
				house = Sprite::create(HOUSE_2_PATH);
			}

			
			int tag = currentTag == 0 ? N_HOUSE-1 : currentTag - 1;
			...
			house->setScale(2.0f);
			house->setPosition(this->getChildByTag(tag)->getPosition().x + this->getChildByTag(tag)->getContentSize().width + house->getContentSize().width - HOUSE_SPEED*delta
				, GROUND_HEIGHT + WALL_HEIGHT);
			//houses.push_back(house);
			this->addChild(house);
		}
		else
			house->setPosition(house->getPosition().x - HOUSE_SPEED*delta, house->getPosition().y);
